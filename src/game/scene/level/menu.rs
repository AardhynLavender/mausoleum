...
use crate::game::modal::modal::{close_modal, make_modal, Modal};
...
pub fn make_menu(world: &mut World, event: &mut EventStore, asset: &mut AssetManager, state: &mut State) {
...
fn close_menu(world: &mut World, event: &mut EventStore, state: &mut State) {
...
impl Systemize for MenuPane {
  fn system(SysArgs { world, scene, event, state, .. }: &mut SysArgs) -> Result<(), String> {
    ...
    if exit {
      close_menu(world, event, state);
    }
    if select {
      match index {
        0 => { close_menu(world, event, state); }
